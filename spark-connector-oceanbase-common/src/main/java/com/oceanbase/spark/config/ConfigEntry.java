...
public class ConfigEntry<T> {
     ...
     * @param version The version that introduces this configuration.
                ...
                value -> {
                    if (!checkValueFunc.apply(value)) {
                        throw new IllegalArgumentException(
                                String.format(
                                        "%s in %s is invalid. %s",
                                        stringConverter.apply(value), key, errorMsg));
                    }
                });
        return this;
    }
    ...
    public ConfigEntry<List<T>> toSequence() {
        ...
        conf.setStringConverter(
                (List<T> val) -> val == null ? null : seqToStr(val, stringConverter));
    ...
    /**
     * Reads the configuration value. If the configuration value is not found, it will try to find
     * the value from the alternatives，which means that newer configurations have higher priority
     * over deprecated ones.
     *
